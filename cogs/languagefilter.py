...
class Automod(commands.Cog):
        ...
        for row in records:
            if row[0] in message_content:
                if message.author.permissions_in(message.channel).manage_messages:
                    ...
                    embedVar = discord.Embed(title="LANGUAGE WARNING", description=f"Issued to **{message.author}**", color=0xe74c3c)
                    ...
                    embedVar.add_field(name="Original Message:", value=f"**{message.content}**")
    ...
    async def banword(self, ctx, word):
        ...
        if result:
            ...
            embedVar = discord.Embed(title="WORD BANNED", description=f"Issued by **{ctx.message.author}**", color=0xe74c3c)
    ...
    async def unbanword(self, ctx, word):
        ...
        if result:
            ...
            channel = self.client.get_channel(int(mod_id))
            embedVar = discord.Embed(title="WORD UNBANNED", description=f"Issued by **{ctx.message.author}**", color=0xe74c3c)
            embedVar.add_field(name="Word:", value=f"**{word}**", inline=True)
            embedVar.add_field(name="Channel:", value=f"**{ctx.channel.mention}**", inline=True)
            embedVar.add_field(name="Time:", value=f"**{ctime()}**", inline=True)
            await channel.send(embed=embedVar)
